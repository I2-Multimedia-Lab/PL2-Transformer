...
if __name__ == '__main__':
    dataset = SegDataset(
            root='/home/zz/synthia/processed_pc',
            meta='/home/zz/synthia/trainval_raw.txt',
            labelweight='/home/zz/synthia/labelweights.npz',
            frames_per_clip=1,
            num_points=16384,
            train=True
    )
    print(len(dataset))
    #data_loader = torch.utils.data.DataLoader(dataset, batch_size=8, shuffle=True, num_workers=16, pin_memory=True)

    print(len(dataset))
    pc1, rgb1, label1, mask1, pc2, rgb2, label2, mask2=dataset[245]
    print(pc1.shape)
    print(rgb1.shape)
    print(label1.shape)
    #np.save("pc1_245.npy",pc1)
    #np.save("rgb1_245.npy",rgb1)
    #np.save("label1_245.npy",label1)
    
    #np.save("pc2_245.npy",pc2)
    #np.save("rgb2_245.npy",rgb2)
    #np.save("label2_245.npy",label2)