...
class P4DConv(nn.Module):
    def __init__(self,
        ...
        conv_d = [nn.Conv2d(in_channels=4, out_channels=mlp_planes[0], kernel_size=1, stride=1, padding=0, bias=bias)]  #通道数变成了3
        if mlp_batch_norm[0]:
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for t in range(self.temporal_kernel_size//2, len(xyzs)-self.temporal_kernel_size//2, self.temporal_stride):                 # temporal anchor frames
            ...
            new_feature = torch.stack(tensors=new_feature, dim=1) #b, 3,1024 n/s 
            if self.temporal_pooling == 'max':
                new_feature = torch.max(input=new_feature, dim=1, keepdim=False)[0] #b 1024 n/s 
            ...
            origin_xyzs.append(xyzs[t])
            new_xyzs.append(anchor_xyz)
            new_features.append(new_feature)
        origin_xyzs = torch.stack(tensors=origin_xyzs,dim=1)
